...
Function Get-LastShutdownTime {
    $ShutdownEvent = Get-WinEvent -LogName System | Where-Object { $_.Id -eq 1074 -or $_.Id -eq 6008 } | Sort-Object TimeCreated -Descending | Select-Object -First 1
    return $ShutdownEvent.TimeCreated
}

# Function to get the last reboot time
Function Get-LastRebootTime {
    $RebootEvent = Get-WinEvent -LogName System | Where-Object { $_.Id -eq 1074 -or $_.Id -eq 6008 } | Sort-Object TimeCreated -Descending | Select-Object -First 1
...
Function ShouldShowToastNotification {
    ...
    # Get the last reboot time and last shutdown time
    ...
    # Calculate the number of days since the last reboot, shutdown, or power-on
    $UptimeDuration = $CurrentDate - $Uptime
    $UptimeDays = [math]::Floor($UptimeDuration.TotalDays)
    
    # Check if the UptimeDays is greater than the threshold
